...
module Cart
  RSpec.describe CartsController, type: :controller do
    ...
    let(:coupon) { create :cart_coupon }
    let(:valid_attributes) { attributes_for(:cart_cart).merge(coupon: { code: coupon.code }) }
    let(:invalid_attributes) { attributes_for(:cart_cart).merge(coupon: { code: 'abbccdd' }) }
